...
// Xoay 270 độ thì quay 90 độ 3 lần ^^
void XoayMaTran1Goc270DoTheoChieuKimDongHo(int a[][MAX], int dong, int cot)
{
    int c[MAX][MAX];
    for(int i = 0; i < dong; i++)
    {
    ...
    for(int i = 0; i < dong; i++)
    {
        for(int j = 0; j < cot; j++)
        {
            printf("%4d", e[i][j] = d[cot - 1 - j][i]);
        }
        printf("\n\n");
    }
}
void XoayMaTran1Goc270DoTheoChieuKimDongHo_2(int a[][MAX], int dong, int cot)
{
...
int main()
{
    ...
    NhapMang(a, dong, cot);
    XuatMang(a, dong, cot);
    printf("\nXoay ma tran 1 goc 270 do theo chieu kim dong ho: \n");
    XoayMaTran1Goc270DoTheoChieuKimDongHo_2(a, dong, cot);
